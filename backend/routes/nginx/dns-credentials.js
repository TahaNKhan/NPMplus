...
router
	...
	.get(async (req, res, next) => {
		try {
			...
			const apiRows = rows.map((row) => ({
				id: row.id,
				provider_id: row.npmplus_provider_id,
				credentials: row.npmplus_credentials,
				name: row.npmplus_name,
				created_on: row.npmplus_created_on,
				modified_on: row.npmplus_modified_on,
			}));
	...
	.post(async (req, res, next) => {
		try {
			...
			const dbPayload = {
				npmplus_provider_id: payload.provider_id,
				npmplus_credentials: payload.credentials,
				npmplus_name: payload.name,
			};
			const result = await internalDnsCredentials.create(res.locals.access, dbPayload);
			const apiResult = {
				id: result.id,
				provider_id: result.npmplus_provider_id,
				credentials: result.npmplus_credentials,
				name: result.npmplus_name,
				created_on: result.npmplus_created_on,
				modified_on: result.npmplus_modified_on,
			};
	...
	.put(async (req, res, next) => {
		try {
			...
			if (payload.provider_id) {
				dbPayload.npmplus_provider_id = payload.provider_id;
			}
			if (payload.credentials) {
				dbPayload.npmplus_credentials = payload.credentials;
			}
			if (payload.name) {
				dbPayload.npmplus_name = payload.name;
			}
